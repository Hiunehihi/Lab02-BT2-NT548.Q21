...
Trong bài này có 2 stack CloudFormation khác nhau:

- `lab02-bt2-pipeline`: stack tạo hệ thống CI/CD, gồm CodePipeline, CodeBuild, S3 artifact bucket, IAM roles và EventBridge trigger.
- `lab02-bt2-infrastructure`: stack tạo hạ tầng chính, gồm VPC, route tables, NAT Gateway, EC2 và Security Groups.

Stack `lab02-bt2-pipeline` được tạo trước vì nó là hệ thống tự động hóa. Sau khi source code được push lên CodeCommit, Deploy stage của CodePipeline mới tạo hoặc cập nhật stack `lab02-bt2-infrastructure`. Như vậy hạ tầng chính không được tạo thủ công trước, mà được triển khai tự động thông qua pipeline đúng theo yêu cầu bài.

### 7.1. Deploy Pipeline Stack

Dùng file `pipeline.yaml` để tạo stack CI/CD `lab02-bt2-pipeline`:
...
Cần thay `YOUR_PUBLIC_IP/32` bằng IP public của máy đang dùng, ví dụ `203.0.113.10/32`.
